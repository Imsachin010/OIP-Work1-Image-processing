...
while True:
    for event in pygame.event.get():
        ...
        if event.type == MOUSEBUTTONUP:
            ...
            rect_min_x, rect_max_x = max(number_xcord[0]-Boundaryinc,0),min(Windowsizex, number_xcord+Boundaryinc)
            rect_min_Y, rect_max_Y = max(number_ycord[0]-Boundaryinc,0),min(Windowsizey, number_ycord+Boundaryinc)
            ...
            if PREDICT:
                ...
                label = str(label[np.argmax(MODEL.predict(image.reshape(28,28,1)))]).title()
